...
**每次任务完成后，必须将本次变更摘要追加到 `docs/project/dev-log.md`。**

### 日志目标

阅读开发日志即可回答两个问题：
1. **做了什么**：改动了哪些文件、新增了什么能力
2. **为什么这样做**：背景、动机、关键决策

### 日志格式

在 `docs/project/dev-log.md` 文件顶部（`<!-- 新条目追加在此处上方 -->` 注释上方）插入新条目，保持时间倒序：

```markdown
## YYYY-MM-DD · Entry NNN · <一句话标题>

### 变更范围
列出涉及的文件或模块（如 `src/agent/run-agent.ts`、`apps/weave-graph-web/`）
...
- 条目编号（Entry NNN）按已有最大编号 +1 递增
- 标题简明扼要，一句话能说清楚的不用两句
- 不需要重复 git commit 消息，日志侧重**原因**，提交消息侧重**结果**
